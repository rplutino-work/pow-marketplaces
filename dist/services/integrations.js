...
exports.getIntegrationByHermesId = getIntegrationByHermesId;
exports.resolveIntegration = resolveIntegration;
exports.createIntegration = createIntegration;
exports.updateIntegration = updateIntegration;
exports.deleteIntegration = deleteIntegration;
exports.syncCredentialsFromHermes = syncCredentialsFromHermes;
...
// CONSULTAS
// ═══════════════════════════════════════════════════════════════════════════
async function getIntegrations(filter) {
    const prisma = (0, database_1.getPrisma)();
    const where = {};
    if (filter?.marketplace_id)
        where.marketplace_id = filter.marketplace_id;
    if (filter?.estado)
        where.estado = filter.estado;
    if (filter?.cliente_domain)
        where.cliente_domain = filter.cliente_domain;
...
async function getIntegrationById(integrationId) {
    ...
    const rulesFormatted = integration.rules.map(rule => {
        let config = {};
        try {
            config = JSON.parse(rule.rule_json);
        }
        catch { }
        return {
            id: rule.id,
...
async function getIntegrationByHermesId(hermesIntegrationId, clienteDomain) {
    const prisma = (0, database_1.getPrisma)();
...
async function resolveIntegration(params) {
    const prisma = (0, database_1.getPrisma)();
    ...
    if (!integration) {
        logger_1.logger.warn(`Integración no encontrada: ${params.hermes_integration_id} desde ${domain}`);
        return null;
    }
    return integration;
}
...
async function createIntegration(data) {
    ...
    logger_1.logger.info(`Integración creada: ${integration.id}`);
    return integration;
}
async function updateIntegration(integrationId, data) {
    const prisma = (0, database_1.getPrisma)();
    const updateData = { updated_at: new Date() };
    if (data.cliente_name !== undefined)
        updateData.cliente_name = data.cliente_name;
    if (data.estado !== undefined)
        updateData.estado = data.estado;
    if (data.hermes_api_url !== undefined)
        updateData.hermes_api_url = data.hermes_api_url;
    if (data.hermes_token !== undefined)
        updateData.hermes_token = data.hermes_token;
    if (data.hermes_enabled !== undefined)
        updateData.hermes_enabled = data.hermes_enabled;
    ...
    logger_1.logger.info(`Integración actualizada: ${integration.id}`);
    return integration;
}
async function deleteIntegration(integrationId) {
    ...
    logger_1.logger.info(`Integración eliminada: ${integrationId}`);
}
// ═══════════════════════════════════════════════════════════════════════════
// CREDENCIALES
// ═══════════════════════════════════════════════════════════════════════════
async function syncCredentialsFromHermes(data) {
    const prisma = (0, database_1.getPrisma)();
    logger_1.logger.info(`Sincronizando credenciales para Hermes ${data.hermes_integration_id}`);
    // Buscar marketplace
...
async function getAccessToken(integrationId) {
    ...
    if (credential.expires_at && credential.expires_at <= new Date()) {
        logger_1.logger.warn(`Token expirado para integración ${integrationId}`);
        return null;
    }
...
async function getIntegrationHealth(integrationId) {
    const prisma = (0, database_1.getPrisma)();
    const integration = await prisma.integration.findUnique({
        where: { id: integrationId },
        include: {
