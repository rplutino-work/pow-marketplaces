...
if (!ENCRYPTION_KEY) {
    logger_1.logger.warn('⚠️ ENCRYPTION_KEY no configurada - las credenciales no estarán seguras');
}
// ═══════════════════════════════════════════════════════════════════════════
...
function encrypt(data) {
    ...
    try {
        ...
        logger_1.logger.error('Error encriptando datos:', { error: error.message });
...
function decrypt(encryptedData) {
    ...
    try {
        ...
        const key = Buffer.from(ENCRYPTION_KEY, 'base64');
        const decipher = crypto_1.default.createDecipheriv(ALGORITHM, key, Buffer.from(data.iv, 'base64'));
        ...
        try {
            ...
            logger_1.logger.error('Error desencriptando datos:', { error: error.message });
