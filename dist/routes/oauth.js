...
const oauthService = __importStar(require("../services/oauth"));
 ...
 */
router.get('/meli/link', async (req, res) => {
    try {
        ...
        logger_1.logger.info(`🔐 Iniciando OAuth para Hermes ${hermes_integration_id} desde ${clienteDomain}`);
        ...
        logger_1.logger.error('Error iniciando OAuth:', { error: error.message });
        res.status(500).json({
...
router.get('/auth/callback', async (req, res) => {
    try {
        ...
        if (error) {
            logger_1.logger.error('Error en OAuth callback:', { error, error_description });
      ...
      `);
        ...
        logger_1.logger.info(`🔐 Procesando callback OAuth con state: ${state}`);
      ...
      `);
        ...
        logger_1.logger.info(`✅ OAuth completado, redirigiendo a: ${redirectUrl.toString()}`);
 ...
 */
router.get('/:marketplace_code/authorize', async (req, res) => {
    logger_1.logger.warn('⚠️ Usando endpoint deprecado, redirigiendo a /meli/link');
    const { integration_id, hermes_url, client_id } = req.query;
    const newUrl = new URL('/api/v1/oauth/meli/link', `${req.protocol}://${req.get('host')}`);
    if (integration_id)
        newUrl.searchParams.set('hermes_integration_id', String(integration_id));
    if (hermes_url)
        newUrl.searchParams.set('hermes_url', String(hermes_url));
 ...
 */
router.get('/:marketplace_code/callback', async (req, res) => {
    logger_1.logger.warn('⚠️ Usando callback deprecado, redirigiendo a /auth/callback');
...
router.get('/integrations/:id/status', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error obteniendo estado de token:', { error: error.message });
...
router.post('/:id/refresh-token', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error en refresh de token:', { error: error.message });
        res.status(500).json({ error: 'Error interno' });
    }
});
/**
...
router.delete('/:id/credentials', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error revocando credenciales:', { error: error.message });
        res.status(500).json({ error: 'Error interno' });
    }
});
