 ...
 */
router.get('/marketplaces/status', async (req: Request, res: Response) => {
  try {
    const prisma = getPrisma();
    ...
    logger.error('Error obteniendo estado de marketplaces:', { error: error.message });
...
router.patch('/integrations/:id', async (req: Request, res: Response) => {
  try {
    ...
    if (hermes_enabled !== undefined) updateData.hermes_enabled = hermes_enabled;
    if (cliente_name !== undefined) updateData.cliente_name = cliente_name;
    ...
    logger.info(`Integración actualizada por admin: ${integration.id}`);
 ...
 */
router.post('/sync/catalog/:integration_id', async (req: Request, res: Response) => {
  try {
    const prisma = getPrisma();
    ...
    logger.error('Error programando sincronización:', { error: error.message });
    res.status(500).json({ error: 'Error interno' });
  }
});
 ...
 */
router.post('/disable/all/:integration_id', async (req: Request, res: Response) => {
  try {
    const prisma = getPrisma();
    ...
    logger.info(`Integración desactivada por admin: ${req.params.integration_id}`);
    res.json({ success: true, message: 'Integración desactivada' });
  } catch (error: any) {
    logger.error('Error desactivando integración:', { error: error.message });
    res.status(500).json({ error: 'Error interno' });
  }
});
 ...
 */
router.post('/resync/orders/:integration_id', async (req: Request, res: Response) => {
  try {
    const prisma = getPrisma();
    ...
    logger.error('Error programando reenvío de órdenes:', { error: error.message });
    res.status(500).json({ error: 'Error interno' });
  }
});
